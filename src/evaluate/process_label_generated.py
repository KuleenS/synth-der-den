...
def combine_with_generated(output_data: str, generated_path: str, training_dataset: str):
    ...
    with open(os.path.join(output_data, f'{training_dataset}', f'train_{training_dataset}.conll'), "r") as f:
        ...
        training_data = [[' ', ' ', '\n'] if x == ['\n'] else x for x in training_data]

        training_data = [x for x in training_data if x != ['DISEASE\n'] and x != ['O\n']]
    ...
    with open(generated_path, "r") as f:
        ...
        generated_data = [[' ', ' ', '\n'] if x == ['\n'] else x for x in generated_data]
...
def label_generated_conll(dataset_folder: str, training_dataset: str, model_output: str, models_to_train):
    ...
    for i in range(len(datasets_modes)):
        ...
        for j in range(len(models_to_train)):
            ...
            output_file = os.path.join(dataset_folder, f"{training_dataset}_{datasets_mode}genlabelled", f"{datasets_mode}generated{models_to_train[j]}label{training_dataset}.conll")

            args = Namespace(
                file = input_file,
                files = None,
                model_checkpoint = os.path.join(model_output, "baselines", f'{training_dataset}', f'{models_to_train[j]}/'),

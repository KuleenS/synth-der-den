...
from src.normalization_models.krissbert.data.bc5dr.bc5dr_utils import bc5dr_utils_process, bc5dr_utils_process_ood

class BCD5DRDataset(torch.utils.data.Dataset):
    ...
    def __init__(self, split: str, ood: bool, omim_to_cui: Dict[str, str], mesh_to_cui: Dict[str, str]) -> None:
        ...
        if self.ood:
            self.docs = bc5dr_utils_process_ood(self.dataset, self.omim_to_cui, self.mesh_to_cui)
        else:
            self.docs = bc5dr_utils_process(self.dataset, split, self.omim_to_cui, self.mesh_to_cui)

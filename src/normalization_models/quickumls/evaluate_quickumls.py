...
def run_evaluation(data, matcher, omim_to_cui, mesh_to_cui):
    ...
    for row in data:
        matches = matcher.match(row[0], best_match=True, ignore_syntax=False)

        if len(matches) > 0:
            predictions.append(matches[0][0]["cui"])
        else:
            predictions.append("")
...
def main(args):
    ...
    print("BCDR5 Disease", run_evaluation(bc5dr_tuples, matcher, omim_to_cui, mesh_to_cui))
    print("NCBI", run_evaluation(ncbi_tuples, matcher, omim_to_cui, mesh_to_cui))
